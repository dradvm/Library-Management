<template>
  <div class="">
    <div class="flex items-center justify-around">
      <div class="text-indigo-600 text-3xl font-medium"></div>
    </div>
    <div class="">
      <div
      ...
      >
        ...
        <div class="flex justify-center" style="width: 25%">Trạng thái</div>
        <div class="flex justify-center" style="width: 25%"></div>
      </div>
        ...
        >
          ...
          <div class="flex justify-center font-medium" style="width: 25%">
            <div
              ...
              :class="{
                'bg-slate-200 text-slate-700': item.trangThai == 'Out Dated',
                'bg-green-500 text-white': item.trangThai == 'Book Returned',
                'bg-primary text-white': item.trangThai == 'Confirmed',
                'bg-red-600 text-white': item.trangThai == 'Rejected',
                'bg-indigo-600 text-white': item.trangThai == 'Waiting Confirm',
              }"
            >
              {{
                item.trangThai == "Out Dated"
                  ...
                  ? "Bị từ chối"
                  : item.trangThai == "Waiting Confirm"
                  ? "Chờ xác nhận"
                  : "Đang mượn"
              }}
            </div>
...
const fetchPagesOfTheoDoiMuonSach = () => {
  theoDoiMuonSachService
    .getPagesOfTheoDoiMuonSachByDocGia({
      theoDoiMuonSachPerPage: theoDoiMuonSachPerPage.value,
      trangThais: trangThais.value,
      maDocGia: authStore.getId(),
    })
